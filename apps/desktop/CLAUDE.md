...
## Auto-start de dependências externas (desde a SPEC-0060, estendido pela SPEC-0061)

Consome o [ADR-0027](../../docs/06-adr/ADR-0027-external-process-lifecycle-management.md) (novo, `Accepted`) — 1º consumidor real do ADR. Com a SPEC-0061, as **três** SPECs candidatas que o ADR nomeava estão entregues (SPEC-0059, painel de rede/busca; SPEC-0060, Ollama; SPEC-0061, container Docker do provedor de busca) e o ADR fica **inteiramente consumido**. `ensureExternalDependencies(env = process.env)`/`releaseExternalDependencies()` (`core-bridge.ts`, instância única de módulo do `DependencyManager` de `@atlas/core`) são chamadas **só** dentro de `app.whenReady()`/`before-quit` de `main.ts` (`void`, sem `await`, ao lado do teardown já existente de `closeAllChatSessions`/`piperTts.shutdown`) — **nenhum canal IPC novo, nenhum diff no renderer** em nenhuma das duas fatias.

Opt-in **só** por variável de ambiente (sem flag, sem GUI — D15 mantida em ambas): `ATLAS_AUTO_START_OLLAMA` (booleano) e `ATLAS_AUTO_START_SEARCH_CONTAINER` (nominal — o nome do container, `''` = desligado). `ensureExternalDependencies` é a **primeira leitura de `process.env` em `core-bridge.ts`**, por isso recebe `env` como parâmetro injetável (testável sem tocar `process.env` global). Cada dependência é resolvida e tratada de forma independente (a falha de uma nunca impede a outra); valor **inválido** em qualquer uma é tratado como **desligado** + uma linha pinada de `console.warn`, nunca uma exceção — divergência deliberada da CLI (que lança `CliUsageError` para o mesmo valor): o ADR-0027(f) proíbe categoricamente que a janela deixe de abrir por causa desta automação. Desfecho `'started'` ⇒ `console.info`; `'failed'` ⇒ `console.warn` com a `reason` — única superfície de transparência desta fatia no desktop (sem painel, sem `#global-alert`, sem diálogo, para as duas dependências). No desligamento, só o que a **própria sessão** iniciou é derrubado, com captura própria por dependência.

**Residual nomeado:** a única fonte de opt-in é uma variável de ambiente, e um app **empacotado** aberto por Finder/Dock tipicamente **não herda** o ambiente do shell — nem o opt-in nem os logs do main process são alcançáveis nesse modo, para nenhuma das duas dependências. Esta fatia do desktop vale, na prática, só para lançamento via terminal/dev (`pnpm --filter @atlas/desktop start`), até uma fatia futura de painel fechar o resíduo — mesmo precedente da SPEC-0055/D17, fechado depois pela SPEC-0059 para o eixo de rede. **Residual adicional da SPEC-0061:** ligar o container **não** concede `netRoots` ao host do endpoint de busca — o usuário continua precisando autorizar pelo painel de Permissões (SPEC-0059); nenhum dos `spawn`/`docker start` tem teto de tempo (achado não-bloqueante do `architecture-reviewer`).
