...
Este arquivo descreve o **estado atual** e as **regras em vigor**. O histórico fatia a fatia vive nas SPECs (`docs/implementation/specs/`, SPEC-0031 a 0062) e em `CLAUDE-ARCHIVE.md` (versão anterior deste arquivo, congelada — não leia no arranque).
...
**Campo do container de busca (desde a SPEC-0062):** dentro de `#search-detail`, abaixo do aviso de host, três IDs novos — `#search-container-input`/`#search-container-apply`/`#search-container-status` — ligam ao gesto `ensureSearchContainer` do `core-bridge` (ver "Auto-start de dependências externas" abaixo). É um gesto **próprio**, fora do mutex de política e de `withSelections`/`composeOverride`: não entra na serialização de `chatTurnInFlight`/`askInFlight`/`micBusy()`/`policyApplicationInFlight`, não encerra sessões de chat vivas, não abre diálogo de consentimento (preencher o nome e clicar **é** o opt-in) e não altera `selectedNetworkAccess()`. O campo nunca é semeado a partir do status/ambiente — só o próprio gesto escreve em `#search-container-status`, com os textos exaustivos de dependência (ver abaixo) prefixados por `⚠️ ` em rejeição.
...
`#panel-system` mostra, nesta ordem: relógio (`#system-clock-date`/`#system-clock-time`, formato `<dia da semana>, DD/MM/AAAA` e `HH:MM:SS`, sem `Intl`) → tokens (`#system-tokens`, quatro desfechos exaustivos sobre `TokenUsageSnapshot`) → CPU/Memória/GPU/Rede (`#system-cpu`/`#system-memory`/`#system-gpu`/`#system-network`) → `#system-dependencies` (desde a SPEC-0062, ver "Auto-start de dependências externas" acima) → `#system-status`. Formatação pinada: separador decimal `,`, milhar `.`, base 1000 em bytes (`B`/`kB`/`MB`/`GB`/`TB`), três textos de indisponibilidade nomeados por `reason`. **Um único `setInterval` de 1000 ms**, criado só quando o painel fica visível e cancelado ao fechar/trocar de painel/`Escape`/backdrop/teardown — fora disso, nenhum timer desta fatia existe. Cada tick atualiza o relógio (`Date` local, sem IPC); a cada **dois** ticks (2000 ms) dispara `metrics.read()`/`tokens.read()`, sem reentrância (tick pulado se uma leitura ainda está em voo) e descartando resposta que chega após o painel fechar. Rejeição de qualquer `invoke` mostra os textos pinados de falha nas células e em `#system-status`, mantendo o painel utilizável — **nunca** alimenta `#global-alert` nem `#presence-core[data-state="error"]` (erro de painel fica no painel). A leitura **não** entra na serialização de gestos: não marca operação em voo, não é bloqueada por turno de chat/`ask` em voo, não é bloqueada por `micBusy()`.

Manifesto de IDs estáticos passou de 77 para **86** (nove novos, todos dentro de `#panel-system`) — depois estendido para 99 pela SPEC-0059 (13 IDs de rede/busca) e para **103** pela SPEC-0062 (4 IDs: 3 do campo de container + `#system-dependencies`). Zero módulo/Tool/Skill/Persona novo; zero mudança em `apps/cli`/`@atlas/runtime`/`@atlas/tools`/`@atlas/permissions`/`@atlas/context`/`@atlas/memory`/`@atlas/persona`/`@atlas/skills`; layout v3.0 (SPEC-0053) intacto.

---

## Auto-start de dependências externas (SPEC-0060/0061, default invertido e gesto de GUI pela SPEC-0062)

Consome o [ADR-0027](../../docs/06-adr/ADR-0027-external-process-lifecycle-management.md) (`Accepted`, inteiramente consumido desde a SPEC-0061) e o [ADR-0028](../../docs/06-adr/ADR-0028-desktop-dependency-autostart-default.md) (`Accepted`, supersessão **parcial** de ADR-0027(g) — só `apps/desktop`, só Ollama). `ensureExternalDependencies(env = process.env)`/`releaseExternalDependencies()` (`core-bridge.ts`, instância única de módulo do `DependencyManager` de `@atlas/core`) continuam chamadas **só** dentro de `app.whenReady()`/`before-quit` de `main.ts` (`void`, sem `await`).

**Ollama — repouso invertido (SPEC-0062, ADR-0028(i)):** `DESKTOP_AUTO_START_OLLAMA_DEFAULT = true`, constante exportada de `core-bridge.ts` e **único** literal que decide esse default no app — `@atlas/core`/`apps/cli` seguem com `autoStartOllama: false`. `ATLAS_AUTO_START_OLLAMA` ausente/vazia agora **liga**; `'false'`/`'0'`/`'off'` continuam a via explícita de desligamento; valor **inválido** continua desligado + `console.warn` pinado (`INVALID_AUTO_START_OLLAMA_ENV_WARNING`, texto inalterado) — só a polaridade do caso ausente mudou.

**Container de busca — fail-closed intacto, com uma segunda porta de entrada (SPEC-0062, ADR-0028(ii)):** `ATLAS_AUTO_START_SEARCH_CONTAINER` segue exatamente como antes (nominal, `''` = desligado, nenhum nome adivinhado). O painel de rede/busca ganha um gesto **próprio** — `ensureSearchContainer(container)` em `core-bridge.ts`, sobre `DependencyManager.ensureSearchContainer` (método aditivo de `@atlas/core`, mesmo caminho privado `inspect`→`start`→polling que o `ensure` de bootstrap já usava, nunca duplicado). Ordem fail-closed: `parseContainerNameSetting` (mesma origem única da SPEC-0061/D5) → nome inválido rejeita com `Error` pinado (`INVALID_SEARCH_CONTAINER_NAME_MESSAGE`), sem tocar a porta; nome vazio resolve `'disabled'` sem tocar a porta; nome válido delega ao manager e grava o desfecho. Preencher o campo e clicar **é** o opt-in explícito — nenhum diálogo de consentimento é aberto (não é concessão de política, D7/D8 da SPEC). **Divergência de mecanismo, documentada no ADR-0028:** o gesto não popula `AtlasConfig.dependencies.autoStartSearchContainer` (populá-lo seria inerte — o `ensure` de bootstrap já assentou em `app.whenReady()`); a equivalência exigida pela cláusula (ii) é de **efeito**, não de mecanismo.

**Posse como conjunto, memoização por nome (SPEC-0062, extensão aditiva de `@atlas/core`):** `ownedContainers` virou `Set<string>` — a sessão pode ligar mais de um nome (bootstrap **e** o gesto de GUI); trocar de nome nunca derruba o anterior; só `release()` (`before-quit`) desliga, um `stopSearchContainer` por nome possuído, captura própria cada um. Duas chamadas concorrentes para o mesmo nome (`ensure` de bootstrap e o gesto, ou dois cliques) compartilham a mesma tentativa via um `Map<string, Promise<DependencyOutcome>>` — desfecho `'started'`/`'already-running'` fica memoizado (chamada seguinte não toca a porta); desfecho `'failed'` é removido do mapa (chamada seguinte tenta de novo). `release()` **drena** o `pending` do `ensure` e todas as tentativas de container ainda em voo (`Promise.allSettled`) antes de ler a posse — sem isso, um `ensureSearchContainer` disparado pela GUI e ainda em polling no instante do `before-quit` registraria posse num conjunto já limpo, deixando um container do Atlas de pé (correção de um bloqueante do 1º gate `architecture-reviewer`). `main.ts` continua chamando `releaseExternalDependencies()` com `void`, sem `await` — a drenagem torna o desligamento **correto**, não **garantido**: um `kill -9` do processo Electron antes dela assentar ainda deixa o container de pé (residual conhecido, não fechado).

**Estado legível pela janela (SPEC-0062):** `readDependencyStatus(): DependencyStatusSnapshot` — síncrona, nunca sobe o Core — devolve `{ ollama: DependencyOutcome | undefined, searchContainers: readonly DependencyOutcome[] }`. `searchContainers` é **lista, de propósito** (não singular): um desfecho por nome distinto tentado nesta sessão, na ordem da primeira tentativa; um nome repetido **substitui** sua entrada no lugar, um nome novo é **acrescentado** — nunca sub-reporta o que a sessão de fato possui (posse é um `Set`). Alimentada por `ensureExternalDependencies` (Ollama + o container do `.env`, se houver) e por `ensureSearchContainer` (GUI); `'disabled'` nunca entra na lista. Dois canais IPC novos: `'atlas:dependencies:read'`/`'atlas:dependencies:search-container'`, expostos em `window.atlas.dependencies.read`/`.startSearchContainer`.

**Painel `Sistema` — auditoria da tentativa (SPEC-0062, ADR-0028(iv)):** `#system-dependencies` (novo, entre `#system-network` e `#system-status`) soma um 3º `invoke` ao mesmo tick de 2000 ms de `metrics.read()`/`tokens.read()` — **nenhum timer novo**. Mostra uma linha para o Ollama (inclusive "ainda verificando" enquanto `undefined`) mais uma linha por entrada de `searchContainers` (ou o texto pinado "nenhum container de busca nesta sessão" com a lista vazia) — prova visual de que a sessão pode ter ligado mais de um container. Rejeição do `invoke` pinta o texto pinado de falha na célula e em `#system-status`, sem tocar `#global-alert`/`#presence-core[data-state]` (mesma regra da SPEC-0054). É a superfície de transparência dentro da janela — os logs de `console.info`/`console.warn` do main process seguem existindo, mas são inalcançáveis num app empacotado.

Opt-in de ambas continua **sem flag de CLI**; do lado da env, cada dependência é resolvida e tratada de forma independente (falha de uma nunca impede a outra); valor **inválido** de qualquer env segue **desligado** + `console.warn` pinado, nunca exceção (ADR-0027(f)); a CLI segue divergindo (`CliUsageError` para o mesmo valor inválido, default `false` para as duas dependências, diff vazio nesta SPEC exceto o fake tipado do teste, D-A1).

**Residual nomeado, restrito ao Ollama pela SPEC-0062:** a única fonte de opt-in do container de busca **por variável de ambiente** continua sujeita ao mesmo resíduo das SPECs 0055/0060 (app empacotado não herda o ambiente do shell) — mas o Ollama não sofre mais esse resíduo (repouso ligado por padrão) e o container ganhou uma segunda via, o campo do painel, que não depende de ambiente nenhum. Nenhum dos `spawn`/`docker start` tem teto de tempo (achado não-bloqueante do `architecture-reviewer`, ainda não fechado). Ligar o container **não** concede `netRoots` ao host do endpoint de busca — o usuário continua precisando autorizar pelo painel de Permissões (SPEC-0059).

---

## Provisionamento assistido de modelo Ollama — painel `Sistema` (desde a SPEC-0063)

Consome o [ADR-0029](../../docs/06-adr/ADR-0029-desktop-model-provisioning-assistant.md) (`Accepted`, segunda metade do pedido de usuário que também gerou o ADR-0028/SPEC-0062). Seção nova **dentro** de `#panel-system`, entre `#system-dependencies` e `#system-status`: catálogo curado e fixo (`src/model-catalog.ts`, novo, sem IO/rede — `MODEL_CATALOG`/`findCatalogModel`/`isInstalledModel`, cinco entradas, `llama3.2` é o único `recommended: true` e coincide, por CA, com `defaultConfig().model.model`).

**Probe de três estados, não dois (D26, correção do bloqueante B1 do 1º gate do `architecture-reviewer`):** `installedProbe: InstalledModelsProbe` (`'pending'`/`'unknown'`/`'known'`) grava `'pending'` na **primeira linha** de `ensureExternalDependencies`, antes do primeiro `await`, e assenta em `'known'`/`'unknown'` quando aquele `ensure` devolve. Sem isso, a leitura de arranque do renderer quase sempre corria antes do bootstrap assentar, e o gatilho proativo — abrir a tela sozinha quando não há nenhum modelo — ficava **inerte** justamente no cenário-alvo (Ollama desligado, auto-start ligado por repouso desde a SPEC-0062). `whenModelProbeSettled()` (novo, único caminho assíncrono de leitura) espera o *deferred* do bootstrap e nunca rejeita; é consumido **uma vez** pelo arranque do renderer, via `'atlas:models:probe'`.

**Gesto de instalar (`installOllamaModel(model)`) e cancelar (`cancelModelInstall()`):** fora do catálogo rejeita sem tocar a porta; um download já em voo devolve `'busy'` **sem** alterar `modelInstallState` e **sem** abortar o download corrente (D9/B2 — um clique num 2º "Instalar" nunca apaga o progresso nem esconde o botão de cancelar do download real). Estado (`'running'` com `completedBytes?`/`totalBytes?`, `'installed'`, `'cancelled'`, `'failed'` com `ModelPullFailureReason`) gravado **antes do primeiro `await`**; `baseUrl` vem sempre de `resolvedDependencyConfig` (a config que o bootstrap de fato usou, D-N2 — nunca um literal novo). Não entra na serialização de `chatTurnInFlight`/`askInFlight`/`policyApplicationInFlight`, não encerra sessões vivas, não toca `selectedPersonaId`/`selectedPermissionRoots`/`selectedNetworkAccess` (D18).

**Fonte única do estado dos controles (B3/D27, correção do 1º gate):** uma flag de módulo do renderer, `modelInstallInFlight`, marcada antes do `invoke` e limpa no `.finally`, é a **única** origem de `disabled`/`hidden` dos botões de instalar e de `#model-install-cancel`, calculada só em `refreshModelControls()` (molde de `refreshAskControls()`). O tick periódico de 2000 ms (mesmo timer de `metrics.read()`/`tokens.read()`/`dependencies.read()`, agora com um 4º `invoke`) pinta **só texto** (`#model-installed-state`, `#model-install-progress`, rótulo `Instalado`) — nunca `disabled`/`hidden`. `#model-install-status` é escrito só pelo desfecho do gesto; `#model-install-progress` só pela leitura periódica.

**Gatilho proativo (ADR-0029(j)):** no arranque, uma chamada a `window.atlas.models.probe()` — `probe.status === 'known'` com lista **vazia** abre o drawer no painel `Sistema` (mesmo `click()` do controle de navegação existente) e move o foco para `#model-catalog`, no máximo uma vez por sessão; `'unknown'` ou lista não vazia não faz nada. Nenhum timer novo, nenhum canal de push — a espera é sobre o *deferred* do bootstrap, trabalho já em curso.

Manifesto de IDs estáticos: 103 → **109** (seis novos, todos dentro de `#panel-system`). `model-catalog.ts` entra no gate de paridade renderer↔módulo (`renderer.speech-parity.test.ts`) como `NOT_MIRRORED` — sete módulos-fonte vigiados. Diff vazio em `selectNetworkAccess`/`selectPermissionRoots`/`ensureSearchContainer`/`readDependencyStatus` e nas quatro portas de diálogo (`ConfirmPort`, `permission-grant-dialog.ts`, `network-grant-dialog.ts`, `persona-delete-dialog.ts` — instalar não abre diálogo de consentimento, o clique com o tamanho já visível **é** o consentimento, ADR-0029(f)).

Residuais nomeados: instalar não atualiza `AtlasConfig.model.model` (modelo instalado pode divergir do configurado); progresso só é visível com o painel aberto (fechar o drawer não interrompe o download, mas some com o indicador); fechar a janela durante um download em voo o interrompe (`release()` aborta antes de drenar); sem teto de tempo para o stream de download (mesma classe do achado da SPEC-0061 sobre `spawn`/`docker start`); superfície só desktop, sem equivalente de CLI (decisão consciente do ADR-0029).
