...
- **Implementação e verificação de uma SPEC `Ready`/`In Progress` são delegadas aos subagents `spec-implementer`/`spec-validator` desde o primeiro passo** (não execução inline no fio principal, e não os skills genéricos `superpowers:executing-plans`/`superpowers:subagent-driven-development`) — ver `docs/04-engineering/ClaudeCodeAutomation.md` antes de implementar ou validar qualquer SPEC. Antes de editar qualquer arquivo em `packages/*/src` ou `apps/*/src` por causa de uma SPEC, pare: esse edit pertence ao `spec-implementer`. Se perceber que começou inline, pare imediatamente e delegue — implementar inline e redelegar depois paga o contexto duas vezes.
