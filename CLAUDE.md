...
- **Desktop** (`apps/desktop`, `@atlas/desktop`; SPEC-0031/0032, [ADR-0019](docs/06-adr/ADR-0019-desktop-electron-stack.md), Electron): primeira interface gráfica do Atlas — abre a **Fase 2** do Roadmap. Round-trip `status` (SPEC-0031) e, desde a SPEC-0032, round-trip `ask` de tiro único (stateless: sobe e desliga o Core por chamada, como `status`), com `ConfirmPort` de diálogo nativo (`dialog.showMessageBox`, fail-closed) para Tools destrutivas e traço visual de `steps` (`formatSteps`); `respond`/multi-turno com sessão do Context Service ficam para o item 2.2. `pnpm --filter @atlas/desktop start` abre a janela. O Core vive só no main process (`src/core-bridge.ts` testável sem Electron + `src/main.ts` casca fina); o renderer fala com o main só por IPC (`contextIsolation: true`, `nodeIntegration: false`). Sem `dist/`: main process via `NODE_OPTIONS=--import=tsx electron ./src/main.ts` (extensão do ADR-0005 ao Electron).
