...
- **Desktop** (`apps/desktop`, `@atlas/desktop`; SPEC-0031/0032/0033, [ADR-0019](docs/06-adr/ADR-0019-desktop-electron-stack.md), Electron): primeira interface gráfica do Atlas — abre a **Fase 2** do Roadmap. Round-trip `status` (SPEC-0031) e round-trip `ask` de tiro único (SPEC-0032, stateless: sobe e desliga o Core por chamada), com `ConfirmPort` de diálogo nativo (`dialog.showMessageBox`, fail-closed) para Tools destrutivas e traço visual de `steps` (`formatSteps`); desde a SPEC-0033, também um **chat visual multi-turno** com o Core mantido **vivo entre turnos** no main process (`openChatSession`/`sendChatTurn`/`closeChatSession`, equivalente GUI do `atlas chat`), sessão viva pelo Context Service (`atlas.context`), reusando os mesmos adapters de diálogo/traço; voz (2.3) e gerência visual de memória/Persona/permissões (2.4) ficam para fatias futuras. `pnpm --filter @atlas/desktop start` abre a janela. O Core vive só no main process (`src/core-bridge.ts` testável sem Electron + `src/main.ts` casca fina); o renderer fala com o main só por IPC (`contextIsolation: true`, `nodeIntegration: false`). Sem `dist/`: main process via `NODE_OPTIONS=--import=tsx electron ./src/main.ts` (extensão do ADR-0005 ao Electron).
