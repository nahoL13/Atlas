...
- **CLI** (`apps/cli`, via `tsx`, sem `dist/`): `atlas status` / `ask` / `chat` (multi-turno com Tools e `confirm`) / `remember` / `forget` / `memory list` / `memory dedupe [--apply]` (consolidação determinística do acervo legado, dry-run por default); config por `flags > env > defaults`; múltiplas raízes `--allow-read`/`--allow-write` (repetíveis; env com `path.delimiter`); `writeRoots` default vazio (opt-in).
