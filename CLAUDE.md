...
- **Desktop** (`apps/desktop`, `@atlas/desktop`; SPEC-0031/0032/0033/0034/0035, [ADR-0019](docs/06-adr/ADR-0019-desktop-electron-stack.md), Electron): primeira interface gráfica do Atlas — abre a **Fase 2** do Roadmap. Round-trip `status` (SPEC-0031) e round-trip `ask` de tiro único (SPEC-0032, stateless: sobe e desliga o Core por chamada), com `ConfirmPort` de diálogo nativo (`dialog.showMessageBox`, fail-closed) para Tools destrutivas e traço visual de `steps` (`formatSteps`); desde a SPEC-0033, também um **chat visual multi-turno** com o Core mantido **vivo entre turnos** no main process (`openChatSession`/`sendChatTurn`/`closeChatSession`, equivalente GUI do `atlas chat`), sessão viva pelo Context Service (`atlas.context`), reusando os mesmos adapters de diálogo/traço; desde a SPEC-0034, também um painel de **gerência visual de memória** — listar e esquecer fatos (`resolveMemorySnapshot`/`forgetFact`, stateless, equivalente GUI de `atlas memory list`/`atlas forget`), primeira das três linhas do item 2.4; desde a SPEC-0035, também **saída de voz (TTS)** — botão "🔊 Ouvir" por resposta do chat, via `window.speechSynthesis`/`SpeechSynthesisUtterance` (Web Speech API embutida no Chromium, offline, vozes locais do SO, sem dependência de runtime nova), num módulo puro `src/speech-output.ts` (`createSpeechOutput`), primeira fatia do item 2.3; entrada por voz (STT)/wake word e as duas linhas restantes de 2.4 (troca de Persona em runtime, permissões por GUI) ficam para fatias futuras. `pnpm --filter @atlas/desktop start` abre a janela. O Core vive só no main process (`src/core-bridge.ts` testável sem Electron + `src/main.ts` casca fina); o renderer fala com o main só por IPC (`contextIsolation: true`, `nodeIntegration: false`). Sem `dist/`: main process via `NODE_OPTIONS=--import=tsx electron ./src/main.ts` (extensão do ADR-0005 ao Electron).
