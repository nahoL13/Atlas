...
- ✅ `candidato · ADR primeiro` — **Auto-gerência de processos externos** — **entregue por inteiro**. Pedido do usuário em 2026-08-22, resolvido pelo [ADR-0027](../06-adr/ADR-0027-external-process-lifecycle-management.md) (`Accepted`, agora **inteiramente consumido**): `createDependencyManager`/`ProcessPort` novos em `@atlas/core`, unidade independente do `Lifecycle` de `createAtlas`, opt-in explícito e fail-closed por dependência, degradando sem nunca bloquear o boot. Nomeou três SPECs candidatas, todas `Done`: a [SPEC-0059](../implementation/specs/SPEC-0059-desktop-network-search-gui.md) (2026-08-22) entregou a 1ª fatia (painel de rede/busca, consumindo só o ADR-0026, não o ADR-0027); a [SPEC-0060](../implementation/specs/SPEC-0060-ollama-auto-start.md) (2026-09-12) entregou a 2ª fatia e o **1º consumidor real do ADR-0027** — auto-start do Ollama sob opt-in explícito (`--auto-start-ollama`/`ATLAS_AUTO_START_OLLAMA` na CLI, `ATLAS_AUTO_START_OLLAMA` no desktop), posse do processo pela autoria do `spawn` (nunca a prontidão), `ollamaBaseUrl` condicionado ao provider efetivo; a [SPEC-0061](../implementation/specs/SPEC-0061-search-container-auto-start.md) (2026-09-13) entregou a 3ª e última fatia — auto-start do container Docker do provedor de busca, extensão aditiva da mesma unidade (`inspectSearchContainer`/`startSearchContainer`/`stopSearchContainer`, `DependencyReport.outcomes` sempre com dois elementos), `AtlasConfig.dependencies.autoStartSearchContainer: string` (opt-in nominal), sem conceder `netRoots` ao ligar o container. Com o ADR-0027 já inteiramente consumido, o pedido do usuário em 2026-09-18 ("Atlas sempre sobe o Ollama sozinho") exigiu supersessão **parcial** e nova, documentada no [ADR-0028](../06-adr/ADR-0028-desktop-dependency-autostart-default.md) (`Accepted`, só `apps/desktop`, só Ollama — cláusula (g) segue intacta para o container); a [SPEC-0062](../implementation/specs/SPEC-0062-desktop-dependency-autostart-default.md) (2026-09-19) implementou esse ADR por inteiro: repouso do Ollama invertido para ligado no desktop (`DESKTOP_AUTO_START_OLLAMA_DEFAULT = true`) e uma 2ª porta de entrada (campo na GUI, `ensureSearchContainer`, método aditivo de `DependencyManager`) para o container, que segue fail-closed. Não é item `gate` — todos os gates da Fase 1 já fecharam antes de este item existir.
