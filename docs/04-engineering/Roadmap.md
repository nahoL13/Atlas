...
- `candidato · ADR primeiro` — **Auto-gerência de processos externos** — pedido do usuário em 2026-08-22, resolvido pelo [ADR-0027](../06-adr/ADR-0027-external-process-lifecycle-management.md) (`Accepted`): `createDependencyManager`/`ProcessPort` novos em `@atlas/core`, unidade independente do `Lifecycle` de `createAtlas`, opt-in explícito e fail-closed por dependência, degradando sem nunca bloquear o boot. Nomeia três SPECs candidatas: painel de rede/busca do desktop, auto-start do Ollama (CLI+desktop) e auto-start do container SearXNG (CLI+desktop). **Parcialmente entregue**: a [SPEC-0059](../implementation/specs/SPEC-0059-desktop-network-search-gui.md) (2026-08-22) entregou a 1ª fatia nomeada (painel de rede/busca, consumindo só o ADR-0026, não o ADR-0027); a [SPEC-0060](../implementation/specs/SPEC-0060-ollama-auto-start.md) (2026-09-12) entregou a 2ª fatia e o **1º consumidor real do ADR-0027** — auto-start do Ollama sob opt-in explícito (`--auto-start-ollama`/`ATLAS_AUTO_START_OLLAMA` na CLI, `ATLAS_AUTO_START_OLLAMA` no desktop), posse do processo pela autoria do `spawn` (nunca a prontidão), `ollamaBaseUrl` condicionado ao provider efetivo. Segue candidata a 3ª fatia: auto-start do container SearXNG. Não é item `gate` — todos os gates da Fase 1 já fecharam antes de este item existir.
