...
# Atualização ([SPEC-0030](../implementation/specs/SPEC-0030-query-aware-memory-recall.md))

A [SPEC-0030](../implementation/specs/SPEC-0030-query-aware-memory-recall.md) faz `MemoryService.prompt(options?: { query?; limit? })` selecionar, com acervo maior que um orçamento fixo (`MEMORY_RECALL_LIMIT = 20`), um **subconjunto** dos fatos em vez do acervo inteiro. A chamada `generate()` de extração pós-turno recebe o **mesmo** valor amostrado do turno (`learner.instruction(memoryValue)`, sem segunda amostragem — preserva a invariante de amostragem única da SPEC-0021), então o learner passa a ver apenas o subconjunto recuperado, não o acervo completo.

- **O custo "sem dedup" registrado acima é menor do que a mitigação por `remember` faz parecer.** O `normalize` interno de `@atlas/memory` (SPEC-0022) é só `trim` → `toLowerCase` → colapso de espaços — cobre apenas **duplicata textual exata** normalizada. Um fato que o learner **reproponha parafraseado** (mesmo sentido, texto diferente) **é gravado como um novo `Fact`**, não é no-op. A frase "nenhuma duplicata é gravada" é válida só para o caso de reproposição **literal**; consolidação semântica de paráfrases segue fatia futura de 1.3.
- **Consequência composta com a ordem de carga (D5/SPEC-0030): fatos aprendidos recentemente ficam sistematicamente fora do subconjunto até casarem lexicalmente com uma consulta.** O completamento do orçamento é sempre por ordem de carga **ascendente** e `remember` sempre `push` no fim — com acervo > 20 e consulta sem overlap, os fatos que entram no piso de orçamento são sempre os **mais antigos**, nunca os recém-aprendidos. Isso significa que, justamente o fato que o learner acabou de propor (e que `extractLearned`/`buildLearnMessages` não viu no turno anterior, por não ter sido selecionado), permanece invisível ao `memoryValue` amostrado nos turnos seguintes até que uma consulta o recupere por relevância — o que aumenta a chance de reproposição (parafraseada ou não) descrita acima.
- **Intactos:** o teto de 3 candidatos por turno, a proveniência (`source: 'learned'`), a mediação da borda (só ela grava) e o mecanismo de proposta/gravação descritos nesta ADR.
