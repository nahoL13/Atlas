...
- **Nota de atualização (2026-07-29, SPEC-0041 concluída — não altera a Decisão).** A [SPEC-0041](../implementation/specs/SPEC-0041-desktop-piper-only-voice-surface.md) restringiu a **superfície de escolha** de voz: quando o Piper está disponível (`PiperTts.isAvailable()` respondendo `true`, exposto ao renderer por um canal IPC novo), o `<select>` do formulário de Persona passa a listar **só** vozes Piper — as vozes do SO deixam de ser escolhíveis pelo usuário. A **Decisão (c)** deste ADR — Web Speech API como fallback fail-closed, nunca removida — **permanece inalterada e em vigor**: ela agora atua só como **rede de segurança interna invisível**, nunca mais como opção visível ao lado do Piper. Além disso, a **precedência** do [ADR-0020(b)](ADR-0020-persona-persistence-voice-binding.md) ("voz da Persona presente e existente ⇒ usa-a") fica **superseded na presença de Piper disponível**: uma `Persona.voiceURI` de SO já persistida deixa de ser honrada enquanto o Piper estiver disponível, ancorado na própria frase deste ADR em (c) — "Piper vira a **primeira** camada de preferência ... ganha o Piper como camada anterior a ambas". O caminho degradado (Piper indisponível) preserva o comportamento das SPECs 0035/0036/0039/0040 sem alteração.
