...
>
> **Nota de atualização (2026-07-21, [SPEC-0027](../implementation/specs/SPEC-0027-memory-fact-retrieval.md)):** a Memory ganha uma operação de **recuperação relevante**, `search(query: string, options?: { limit?: number }): readonly Fact[]`, membro **aditivo** de `MemoryService`. Puro e síncrono, opera só sobre os `facts` já carregados em memória — nenhum IO, nenhum `load`/`save`, mesmo molde de `list`/`prompt`. Reusa o mesmo `normalize(text)` interno das SPECs 0022/0023 para tokenizar consulta e fato; ranking determinístico por overlap de tokens (`score = |tokens(query) ∩ tokens(fact)|`), exclui `score = 0`, ordena por `score` desc, empate desempatado pela ordem de carga (índice ascendente em `facts`, mesmo critério de desempate do `dedupe`). `options.limit` opcional corta os `N` primeiros já ordenados; ausente devolve todos os fatos com overlap. Consulta vazia ou sem tokens após `normalize` devolve `[]`. O retorno é `Fact[]` homogêneo com `list()`, **sem** expor scores — o ranking é detalhe interno. **Não** altera `prompt()` nem o caminho de injeção na geração (o `memoryPrompt` do Cognitive continua recebendo o dump de todos os fatos via `prompt()`); consumir `search` a partir da consulta do turno no Cognitive Core é fatia futura explícita. A autoridade exclusiva da Memory sobre o estado persistente (Artigo 11) não muda — a CLI (`atlas memory search "<consulta>"`) só invoca `search` e renderiza `Fact[]`, nunca toca `normalize`/`storage` diretamente.
>
> **Nota de atualização (2026-07-22, [SPEC-0029](../implementation/specs/SPEC-0029-episodic-project-memory.md)):** a Memory deixa de tratar todo conhecimento persistente como uma lista homogênea e passa a **classificar cada registro em uma categoria de conhecimento** — `MemoryCategory = 'fact' | 'episode' | 'project'`, exportado por `@atlas/contracts`. `Fact` ganha dois campos **opcionais** (`category?`, `subject?`); ausência de `category` ≡ `'fact'` (acervo legado não é migrado nem revalidado no `load`). `remember` ganha um terceiro parâmetro opcional (`options?: { category?; subject? }`), continua **100% compatível** com os chamadores de 1/2 argumentos, e passa a **garantir dentro do módulo** (não na borda) a invariante "memória de projeto exige projeto": rejeita com `MemoryError`, antes de qualquer mutação ou `storage.save`, tanto `category: 'project'` sem `subject` válido quanto `subject` presente com categoria diferente de `'project'` (um `subject` que colapse para vazio após `normalize` conta como ausente/inválido). A **chave de duplicata** (SPEC-0022/0023) passa a incluir categoria e `subject` normalizados, para o mesmo texto em categorias/projetos diferentes não colapsar; `dedupe` reusa a mesma chave. `list` ganha um filtro opcional por categoria (`category` ausente ≡ `'fact'`); `search` **não muda**. `prompt()` passa a compor **seções por categoria**, em ordem fixa `fact → project → episode`, separadas por linha em branco, com a seção `fact` **byte a byte idêntica** à anterior a esta SPEC (não-regressão) e a seção `project` agrupada por `subject` (subcabeçalho `[projeto <subject>]`, ordem de primeira ocorrência na ordem de carga). A autoridade exclusiva da Memory sobre o estado persistente e sobre as invariantes do modelo (Artigo 11) não muda — a CLI (`atlas remember ... --category <c> [--subject <p>]`, `atlas memory list [--category <c>]`) só antecipa a validação com `CliUsageError` amigável, invoca e renderiza; a garantia real vive em `@atlas/memory`. O Learner (SPEC-0020) continua produzindo só `category: 'fact'`; nenhuma mudança em `@atlas/cognitive`/`@atlas/core`.
