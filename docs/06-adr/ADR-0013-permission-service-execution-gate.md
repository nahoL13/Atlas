...
# Atualização ([SPEC-0015](../implementation/specs/SPEC-0015-permission-symlink-hardening.md))

A SPEC-0015 fechou a limitação de symlink registrada desde esta decisão ("Custos e riscos", "Alternativas Consideradas") — **sem** reabrir nenhuma das duas escolhas estruturais desta ADR (portão puro; `evaluate` síncrono):

- **Contenção lexical passou a operar sobre caminhos reais (`realpath`), não mais sobre strings apenas resolvidas por `path.resolve`.** Um symlink dentro de uma `readRoot`/`writeRoot` legítima apontando para fora do diretório permitido, que antes escapava da política, agora é bloqueado — vale igual para `read`/`write`/`delete`.
- **`evaluate` continua puro e síncrono.** O IO de resolução de caminho foi isolado numa nova **porta injetável síncrona**, `PathResolverPort` (`realpathSync(path): string`, interna a `@atlas/permissions`, mesmo critério de placement de `FsReadPort`/`FsWritePort`/`ConfirmPort` — sem 2º consumidor real, não sobe a `@atlas/contracts`), com `nodePathResolverPort()` default sobre `node:fs` `realpathSync` (síncrona, não `fs/promises`). `evaluate` **não** virou `Promise`; o Runtime **não mudou uma linha** — segue chamando `permissions.evaluate(requirement)` sem `await`.
- **Algoritmo do ancestral existente mais profundo.** Como `realpathSync` lança `ENOENT` para caminhos que ainda não existem (o caso comum de `write_file`/`mkdir`/`append_file`/`delete_file` operando sobre algo novo dentro de uma raiz válida), a resolução sobe pelos diretórios pais até o primeiro segmento existente, resolve o `realpath` desse ancestral e recompõe por cima os segmentos ainda inexistentes — preservando "criar algo novo dentro de uma raiz válida é permitido" sem regressão. A lógica de subir até existir vive em `permission-service.ts`, não na porta (a porta só expõe `realpathSync` cru).
- **`readRoots`/`writeRoots` são resolvidas uma única vez, na criação do serviço** (mesmo algoritmo do ancestral existente aplicado às raízes) — não a cada `evaluate`. Uma raiz que ainda não existe no disco, ou que é/contém um symlink, é resolvida corretamente nesse momento; o alvo em `evaluate` é sempre comparado contra a raiz **já resolvida**, nunca contra a raiz bruta.
- **Fail-closed em toda resolução.** Qualquer erro de resolução ≠ `ENOENT` (ex.: `EACCES`) vira `{ verdict: 'blocked', reason }` em `evaluate`, sem lançar. Na criação, se a resolução de **uma** raiz falha com erro ≠ `ENOENT`, **apenas aquela raiz** fica fail-closed (nada é considerado contido nela) — as demais raízes seguem funcionando e `createPermissionService` nunca lança (decisão confirmada com o humano: mais restritivo derrubaria capacidades legítimas não relacionadas ao erro).
- **TOCTOU segue fora de escopo.** A resolução do alvo em `evaluate` e o uso subsequente pela Tool acontecem em momentos distintos; um symlink trocado entre a avaliação e a execução não é tratado por esta fatia — limitação remanescente, candidata a endurecimento futuro.
