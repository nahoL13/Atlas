...
# Atualização ([SPEC-0059](../implementation/specs/SPEC-0059-desktop-network-search-gui.md))

A SPEC-0059 fecha o residual **nomeado** "painel de rede/busca na GUI", deixado em aberto pelas atualizações das SPECs 0055 e 0057, **sem reabrir nenhuma cláusula (a)–(e)**: diff vazio em `@atlas/permissions`, `@atlas/contracts`, `@atlas/core`, `@atlas/tools`, `netRoots`, `evaluate`, `ResourceType`/`ResourceRef`, `isContained`.

- **`apps/desktop` passa a oferecer, pela janela, o mesmo dado de configuração que a CLI já oferecia** (`--allow-net`/`ATLAS_ALLOW_NET`, `--search-url`/`ATLAS_SEARCH_URL`): um painel dentro de `#panel-permissions` (molde direto da SPEC-0038) lista os hosts autorizados e o endpoint de busca em vigor, permite acrescentar/remover hosts e definir/limpar o endpoint, e aplica a mudança de uma vez, sob consentimento explícito por host **novo** (quarta porta fail-closed do app, `network-grant-dialog.ts`).
- **A política chega ao Permission Service só pelo caminho já existente** (`createAtlas` → `loadConfig` → `createPermissionService`): o app nunca chama `evaluate`, nunca compara host, nunca faz requisição — não reimplementa nenhuma parcela da autoridade deste ADR. A validação de hostname/URL segue exclusivamente em `loadConfig` (ADR-0006); o app só a consulta por dry-run antes de aplicar.
- **Consentimento por host, nunca por Tool** (confirma (b) na prática, do lado do consumidor): o diálogo nomeia as Tools de rede existentes hoje (`http_get`/`web_search`) só para tornar o alcance concreto ao usuário, mas declara explicitamente que a concessão vale para **qualquer** Tool de rede que alcance aquele host — coerente com a política ser por host, nunca por Tool.
- **Dois residuais ampliados por esta fatia, registrados sem fechamento**: (i) os três diálogos nativos de consentimento do app (agora quatro) não têm `BrowserWindow` pai (residual já nomeado em `apps/desktop/CLAUDE.md`, "diálogo fantasma") — com o mutex de aplicação de política que esta SPEC introduziu (D16, compartilhado entre o gesto de rede e o de escrita), um diálogo perdido atrás da janela passa a travar as DUAS aplicações de política (FS e rede), não mais só uma; candidato futuro nomeado: diálogos nativos modais com `BrowserWindow` pai. (ii) o texto do diálogo de consentimento de rede enumera nominalmente `http_get`/`web_search` em vez de uma descrição genérica — decisão correta hoje (Decisão D18 da SPEC), mas quando uma terceira Tool de rede for criada esse texto fica desatualizado, e nenhum gate mecânico força a atualização (diferente do gate de paridade renderer↔módulo, que vigia réplicas de código, não texto de diálogo).
- **Nenhum dos residuais herdados de SPEC-0055/SPEC-0057 é fechado ou agravado além do exposto**: DNS rebinding/IP resolvido não validado, redirect não seguido, ausência de wildcard de subdomínio, concessão por host (nunca por porta/caminho), exfiltração pela URL/consulta que o portão não julga, injeção indireta de prompt (mitigada pela SPEC-0058, não fechada), assimetria com o `@atlas/model-gateway` (egress sem portão).
- A configuração aplicada pela janela é **estado de sessão da app**, não persistente (Artigo 11) — mesma fronteira que a SPEC-0038/D2 já registrou para `readRoots`/`writeRoots`.
- Fecha o residual nomeado "painel de rede/busca na GUI"; não fecha o item 1.4 do Roadmap — seguem abertos execução de comandos sob o Permission Service, redirect por hop, wildcard de subdomínio, IP resolvido e exfiltração via URL/consulta.
