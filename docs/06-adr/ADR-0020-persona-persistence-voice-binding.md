...
- **Nota-ponteiro (2026-07-31, SPEC-0043 — não altera a Decisão).** A hierarquia de fallback de voz da Decisão (b) volta a valer **também no renderer** — não só no módulo puro — **no caminho degradado (Piper indisponível)**: `createSpeechOutputGlue` passa a receber `preferredVoiceURI`, fechando a deriva em que o glue do renderer ignorava a Persona ativa e sempre soava a 1ª voz local. **Em modo Piper-only, a precedência de (b) segue superseded pela [SPEC-0041](../implementation/specs/SPEC-0041-desktop-piper-only-voice-surface.md)/[ADR-0021(c)](ADR-0021-piper-tts-local-voice-engine.md)** — a nota acima não é revertida nem afrouxada, esta apenas restaura (b) onde ela já vigorava (o caminho degradado).
