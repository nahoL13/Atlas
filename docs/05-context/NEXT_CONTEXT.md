...
**Fase 2 (`apps/desktop`) em andamento; Fase 1 fechada por inteiro, mas com candidatos ainda sendo entregues em paralelo** (item 1.4, resta só execução de comandos sob o Permission Service, `ADR primeiro`). Itens 2.1, 2.2 (1ª linha), 2.3 e 2.4 fechados; wake word (2.3, "candidato, não comprometido") segue em aberto. *Observabilidade do Ambiente* — exceção consciente sem item de Roadmap — foi entregue por inteiro pela SPEC-0054. [ADR-0027](../06-adr/ADR-0027-external-process-lifecycle-management.md)/[ADR-0028](../06-adr/ADR-0028-desktop-dependency-autostart-default.md)/[ADR-0029](../06-adr/ADR-0029-desktop-model-provisioning-assistant.md) (auto-gerência de processo externo, seu repouso no desktop, e instalação assistida de modelo) estão **todos implementados por inteiro** (SPECs 0059–0063). A SPEC-0064 fechou o achado de watchdog aberto desde a SPEC-0061 — hardening de `@atlas/core`, não item de Roadmap.

Últimas três fatias (detalhe completo em `PLATFORM_STATE.md` e na SPEC de cada uma):

- **SPEC-0064** `Done` (2026-09-21, Perfil micro) — teto de tempo (watchdog) nas operações de `ProcessPort` implementadas por `nodeProcessPort()`: `startOllama`/`inspectSearchContainer`/`startSearchContainer`/`stopSearchContainer` ganham orçamento pinado (5 s/5 s/30 s/30 s) que mata **e desacopla** (`kill()`+`unref()`) o filho no estouro, nunca segurando o event loop do processo pai; `pullOllamaModel` ganha teto de **inatividade** (120 s, nunca duração total) com cancelamento humano preservado por checagem síncrona de `signal.aborted`. Nenhum desfecho/interface novo; diff vazio em `dependency-manager.ts`/`@atlas/contracts`/`apps/desktop`; único texto de CLI ampliado (aviso de `'docker-unavailable'`). Não fecha item de Roadmap. Detalhe: `packages/core/CLAUDE.md`.
...
Suíte atual: **2178 testes / 105 arquivos**. `lint`/`typecheck`/`test`/`format:check` verdes.
...
- **Teto de parede único para `ensure()`** (`@atlas/core`) — residual nomeado pela SPEC-0064: o watchdog por chamada (SPEC-0064) limita cada operação, mas o polling de prontidão do manager ainda multiplica por tentativas (~237 s no pior caso patológico); exigiria relógio injetável novo no manager e revisão de desfechos pinados pelas SPECs 0060/0061.
...
- Candidatos menores: lista completa em `apps/desktop/CLAUDE.md`, seção "Candidatos futuros já nomeados".
...
- **SPECs, planos e lições**: `docs/implementation/` · **ADRs**: `docs/06-adr/` (0001–0029)
