...
Atualizado em: 2026-09-12 (SPEC-0060)
...
- **SPEC-0060** `Done` (2026-09-12) — auto-start do Ollama sob opt-in explícito, 1º consumidor real do [ADR-0027](../06-adr/ADR-0027-external-process-lifecycle-management.md): `createDependencyManager`/`ProcessPort` (`@atlas/core`, independente do `Lifecycle`), disparado uma vez por processo na CLI (`--auto-start-ollama`/`ATLAS_AUTO_START_OLLAMA`) e uma vez por sessão de app no desktop (`ATLAS_AUTO_START_OLLAMA`, sem GUI); posse do processo pela autoria do `spawn`, não pela prontidão (daemon lento nunca órfão); health-check fora de `evaluate`/`netRoots` (ADR-0027(a)). Diff vazio nos demais `packages/*`/renderer. **Não** fecha o item 1.4. Detalhe: `packages/core/CLAUDE.md`.
...
Suíte atual: **1865 testes / 101 arquivos**. `lint`/`typecheck`/`test`/`format:check` verdes.
...
- **Auto-start do container SearXNG** (CLI + desktop) — 3ª SPEC nomeada pelo [ADR-0027](../06-adr/ADR-0027-external-process-lifecycle-management.md); a 2ª (auto-start do Ollama) foi entregue pela SPEC-0060 (`createDependencyManager`/`ProcessPort` já existem em `@atlas/core`, a estender por união discriminada aditiva); no desktop, só tem efeito prático agora que a SPEC-0059 entregou o painel de rede/busca (host do container ainda precisa ser autorizado em `netRoots` pelo usuário).
