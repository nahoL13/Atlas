...
Atualizado em: 2026-08-05 (SPEC-0051)
...
- **SPEC-0051** `Done` (2026-08-05) — gesto de escape: botão "Cancelar" por painel (`#ask-cancel`/`#chat-cancel`) → `window.atlas.cancel()` → `cancelInFlightOperation()` (síncrona, `{ cancelled: boolean }`, idempotente), que abandona a operação cancelável em voo e faz a promessa rejeitar na hora, com mensagem pinada. `busySessions`/`inFlightOperations` viram **um** registro `Set<OperationRecord>` e **quatro predicados nomeados** (`hasInFlightOperation`/`hasActiveOperation`/`hasAbandonedTurnForSession`/`hasChatTurnOperation`). `ConfirmPort` do chat envolvido **por sessão** (alcance real da porta, injetada em `openChatSession`), pegajoso enquanto houver turno abandonado não assentado; do `ask`, por operação. Quarentena de sessão: `sendChatTurn` recusa turno novo na sessão afetada até o trabalho abandonado assentar. **Desistência, não cancelamento real** — o Core segue sem `AbortSignal`; dono previsto do cancelamento real é o Runtime/Task Manager. Zero diff em `packages/*`/`apps/cli`.
- **SPEC-0050** `Done` (2026-08-04) — fecha o resíduo D7 da SPEC-0049: `resolveAskSnapshot` e `sendChatTurn` no `core-bridge` passam a **ler** `hasInFlightOperation()` como guarda de entrada, tornando "um round-trip contra o Core por vez" estrutural no main process (antes garantia exclusiva do renderer). Zero diff em `packages/*`/`apps/cli`/renderer/`main.ts`/`preload.cjs`.
- **SPEC-0049** `Done` (2026-08-03) — fecha as duas direções residuais da SPEC-0048: `#ask-form` entra na serialização de gestos (`id="ask-submit"` + `refreshAskControls()`, recusa silenciosa — D4) e ganha `.catch`, pintando `⚠️ <mensagem>` no `#ask-result` em vez de deixar a rejeição de `atlas.ask` virar unhandled rejection. Zero diff em `packages/*`/`apps/cli`/`core-bridge.ts`.

Suíte atual: **1001 testes / 75 arquivos**. `lint`/`typecheck`/`test`/`format:check` verdes.
...
- **Cancelamento cooperativo real no Runtime/Task Manager** (`AbortSignal`/fila/retry/timeout/estado `cancelled`) — dono já atribuído pelo Module Catalog, hoje não implementado; a SPEC-0051 entregou só a desistência dentro de `apps/desktop`. Atravessa `@atlas/contracts` e ≥ 3 módulos ⇒ **exige ADR novo + decisão humana**.
- **Diálogos nativos modais com `BrowserWindow` pai** — fecharia o "diálogo fantasma" da SPEC-0051 (um `dialog.showMessageBox` já aberto no momento de um cancelamento permanece na tela sem efeito) e a origem da corrida A7 da SPEC-0038; fatia própria, apontada pelo gate da SPEC-0051.
- **Canal de push (`webContents.send`) avisando o renderer quando o trabalho abandonado assenta** — hoje todos os canais são `invoke`/`handle`; sem ele, painéis de configuração voltam a parecer habilitados após o cancelamento e podem recusar até assentar (SPEC-0051/D12).
- **Mensagem de recusa distinguindo operação abandonada de ativa** — hoje `selectPermissionRoots`/`updatePersona` recusam com a mesma mensagem nos dois casos (SPEC-0051).
- **Liberar a quarentena de sessão** se o trabalho abandonado nunca assentar (candidato (iv) do DoD da SPEC-0051) — hoje essa conversa só volta a aceitar turnos se a app reabrir.
- Candidatos remanescentes: **pinar `micBusy()` fora da condição de `#ask-submit` por teste explícito** (D5 da SPEC-0049, hoje só decisão documentada); **uniformizar `selectPersona` para `hasInFlightOperation()`** em vez da condição parcial atual (D12 da SPEC-0050) — hoje um `ask` em voo não bloqueia a troca de Persona, embora bloqueie a edição da Persona ativa; mudaria comportamento de um caminho de configuração não pedido, merece aferição de UX própria.
...
- **Smoke visual/sonoro das fatias desktop pendente de confirmação humana** — 18 fatias visuais seguidas (SPEC-0031 a 0051). O shell de automação não tem WindowServer; a SPEC-0040 acrescentou a ausência do binário Piper; a SPEC-0046 acrescenta a ausência de microfone e do binário `whisper-cli`. Não bloqueia fechamento documental, mas **nenhuma fatia visual/sonora foi confirmada de fato em ambiente real**. Lista item a item no CA 25 da SPEC-0040, ampliada pelas SPECs 0041/0043/0046. A SPEC-0045 cobriu `renderer.js` por DOM de teste (jsdom) — prova lógica e fiação, **não** pixel nem som; não fecha esta pendência (APIs de voz seguem dubladas). Pendente também: captura real de microfone, transcrição real em PT-BR, latência do modelo `small`, o diálogo nativo de permissão do macOS (e o tempo de leitura que motivou o rearme do watchdog), a negativa do usuário nesse diálogo, eco com alto-falante aberto, e — desde a SPEC-0051 — os dois botões "Cancelar", o aviso pinado de cancelamento e o comportamento do diálogo nativo "fantasma" que fica na tela sem efeito após um cancelamento.
