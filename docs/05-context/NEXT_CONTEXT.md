...
- `@atlas/core` compõe `createModelGateway(config.model, { fetch })` + `createMemoryService({ storage })` + `createCognitiveCore({ gateway, personaPrompt, memoryPrompt? })` + `createContextService()` + `createPersonaService()` e expõe `atlas.cognitive`/`atlas.context`/`atlas.persona`/`atlas.memory`; `createAtlas(options, { fetch, memoryStorage? })` injeta `fetch` (testes sem rede) e `memoryStorage` (testes sem disco; default = `createFileMemoryStorage(config.memory.path)`). `config.model`, `config.persona` (contra `PERSONA_IDS`) e `config.memory.path` validados no `loadConfig`.
- Suíte completa verde na última verificação (2026-07-14): `pnpm install && pnpm lint && pnpm format:check && pnpm typecheck && pnpm test` (**121 testes, 20 arquivos**).
- Working tree limpa; branch única `main`, **sem remote** (GitHub/CI ainda não decididos).

---

# Próximo Trabalho: SPEC-0010 (a definir)

A fundação do MVP (workspace + core + CLI + acesso a modelos), a **primeira resposta cognitiva ponta a ponta** (SPEC-0005), a **conversa interativa multi-turno** (SPEC-0006, `atlas chat`), o **Context Service** (SPEC-0007, detentor da conversa por sessão), o **Persona Service** (SPEC-0008, identidade Jarvis injetada na geração) e o **Memory Service** (SPEC-0009, fatos/preferências persistentes injetados na geração) estão entregues. A próxima SPEC ainda **não foi escolhida** — decidir no brainstorming. Candidatas plausíveis (nenhuma comprometida — o projeto não tem roadmap):

- **Próximas etapas do ciclo cognitivo**: Planner / Runtime — evoluir o `ask`/`respond` para orquestrar Planejamento/Execução/Observação/Aprendizado (hoje colapsados). Inclui modelar o **contexto de ambiente** (cwd/repo/branch/arquivos) que o Cognitive/Planner poderão ler no futuro (ADR-0009 deixou essa leitura fora do escopo da SPEC-0007).
- **Próximas fatias do Memory Service**: memória **episódica**, memória de **projetos**, **busca**/indexação, **retenção**/classificação, **relações** entre informações (Module Catalog); **aprendizado automático** (o Cognitive grava fatos sozinho); **leitura/gravação ao vivo** de memória numa sessão `chat` aberta (`/lembrar`, `/esquecer`) — deixados fora de escopo da SPEC-0009.
...
- **TypeScript pinado em `^5`**: typescript-eslint 8.63 quebra com TS 7.0.2 (probes falharam em 2026-07-10, 2026-07-11, 2026-07-12, 2026-07-13 (SPEC-0007), 2026-07-13 (SPEC-0008, quinto probe) **e 2026-07-14 (SPEC-0009, sexto probe)**, sempre o mesmo `TypeError: Cannot read properties of undefined (reading 'Cjs')` em `typescript-estree`). Encaminhamento: parar de repetir a cada SPEC — vincular a um gatilho externo (release do typescript-eslint que declare suporte ao TS7) em vez de re-probar por hábito.
...
- SPECs/planos/lições: `implementation/` · ADRs: `docs/06-adr/` (0001 monorepo, 0002 TS/Node, 0003 composition root, 0004 composição manual, 0005 execução de apps via tsx, 0006 precedência de config, 0007 promoção do contrato do Model Gateway, 0008 conversa como dado, 0009 Context Service como store de valor, 0010 Persona injetada na geração, 0011 Memory Service persistência por porta injetável).
- CLI: `apps/cli` (`@atlas/cli`) — `main.ts` (casca) → `run()` → gateways locais + comandos `status`, `ask` (`commands/ask.ts`), `chat` (`commands/chat.ts` + `gateway/line-reader.ts`), `remember`/`forget`/`memory` (`commands/`). `--memory-path`/`ATLAS_MEMORY_PATH` selecionam o arquivo de memória.
- Cognitive Core: `packages/cognitive` (`@atlas/cognitive`) — `createCognitiveCore({ gateway, personaPrompt? })` → `ask(objetivo)` + `startConversation()`/`respond(conversation, input)`; system prompt composto (`personaPrompt` + `memoryPrompt` + `TASK_FRAMING`, identidade → memória → tarefa, ADR-0010/0011, deixou de ser neutro); sem estado (conversa é dado — ADR-0008).
