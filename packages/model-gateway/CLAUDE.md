...
- `OLLAMA_DEFAULT_BASE_URL` (SPEC-0060/ADR-0027): a constante `DEFAULT_BASE_URL`, antes privada de `providers/ollama.ts`, passa a ser **exportada** (e re-exportada por `index.ts`) e consumida por `@atlas/core` (`resolveDependencyConfig`, auto-start do Ollama) como a única origem do endereço default — zero mudança de comportamento, o provider `local` continua a usar a mesma constante.
