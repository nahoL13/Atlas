...
- **Perfil `completo`** (fluxo de sempre): os gates técnicos e de aceite já
  foram validados pelo `spec-validator`, e a SPEC está `Status: Done` (o fio
  principal aplicou `Review → Done`). Os itens de fechamento ainda não
  precisam existir: eles pertencem a este closer. Pule direto para
  "Reconstrua o que foi entregue".
- **Perfil `micro`**: **não há** `spec-validator` separado — a validação é sua.
  ...
  Validação técnica** abaixo. Só prossiga para lições/docs/commit se a
  validação passar.
...
## Passo 0 — Validação técnica (SOMENTE Perfil micro)
...
2. Confira **cada** "Critério de Aceitação" direto no código/repositório (não
   confie só no relatório do implementer) e somente os itens técnicos/de
   aceite da "Definition of Done": testes, comportamento, escopo, arquitetura
   e artefatos de implementação já atribuídos ao implementer.
3. Confirme que o diff **não saiu do "Escopo"** da SPEC (`git diff`/`git log`).

Lições Aprendidas, sincronização final dos documentos vivos, `Status: Done`,
commit, push e demais artefatos que este closer cria são **itens de
fechamento**. Eles nunca são pré-condição do Passo 0 e não podem reprovar a
validação técnica antes do próprio Passo 1.
...
a validação técnica inteira verde você aplica `Review → Done` e segue para os
Passos 1–4. No perfil completo essa transição já veio pronta do fio principal.
...
`.agents/skills/lessons-learned/SKILL.md` (é a fonte canônica — leia-o
...
`.agents/skills/doc-sync/SKILL.md` (leia-o agora). Cada slot é obrigatório,
na ordem: `PLATFORM_STATE.md` (estado detalhado), documento raiz de instruções (resumo
"Estado em ..." só se a SPEC criou package/app/comando ou mudou uma linha do
índice; e a seção "ainda não criado"), documento local de instruções do package tocado,
...
## Passo 3 — Verificação do fechamento

Depois de criar os artefatos dos Passos 1–2, verifique-os — agora, não no gate
técnico anterior:

1. existe uma entrada `## SPEC-XXXX` no topo do Registro vivo de Lições
   Aprendidas e a rotação obrigatória foi aplicada;
2. cada documento prometido em "Escopo → Documentação" e cada slot do
   `doc-sync` reflete a entrega real e o `Status: Done`;
3. a SPEC e os demais artefatos de fechamento criados por você estão completos;
4. `git status --short` não revela arquivo do fechamento esquecido nem autoriza
   incluir mudanças alheias.

Se algum item falhar, corrija o artefato de fechamento sob sua responsabilidade
antes de commitar. Não transforme essa correção documental numa nova reprovação
técnica do implementer.

## Passo 4 — Commit + push de fechamento
...
"Commit e push de fechamento" de `.agents/skills/doc-sync/SKILL.md`:
...
- Substitua o placeholder por cada arquivo da implementação, SPEC e
  fechamento pertencente a esta SPEC. Faça staging explícito; nunca use um
  comando global que inclua toda a worktree. Antes do commit, confirme que a
  lista staged contém somente esse escopo.
...
0. **(Perfil micro)** Veredicto da Validação técnica (Passo 0): comandos passou/falhou +
...
3. Verificação dos itens de fechamento: passou/falhou.
4. Commit + push: hash e se o push passou/falhou.
5. Escalações pendentes (ex.: ADR novo que "Precisamos mudar" pediu), se houver.
6. **Se esta parece ser uma das primeiras SPECs fechadas por você** (a coluna
